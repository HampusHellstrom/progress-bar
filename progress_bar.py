...
class ProgressBar(object):
    ...
    def _printCurrentProgress(self, text=''):
        ...
        if self.currentProgress == self.iterations:
            loadString = '[%s]' % ('=' * self.barLength)
            print(self.__progN__ % self.currentProgress, loadString)
            print('Complete! Finished in %s.' % self._time_to_string(time.time() - self._startTime))
        elif currentTime - self._lastPrintTime < self._minTimeBetweenPrints:
            pass
        else:
            arrow = '=' * int(self.barLength * self.currentProgress / self.iterations) + '>'
            dots = '.' * (self.barLength - len(arrow))
            ...
            remainingTime = self._time_to_string(remainingTime)
